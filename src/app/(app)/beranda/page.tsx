...
import { LogIn, Folder, FolderPlus, Users, MoreVertical, UserPlus, Trash2, LogOut, FileCog, Crown } from 'lucide-react';
          ...
          <div className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {projects.map((project) => {
                ...
                return (
                  <Card key={project.id} className="h-full flex flex-col hover:border-primary transition-colors duration-200 relative group">
                    <Link href={`/proyek/${project.id}`} className="flex flex-col flex-grow">
                      <CardHeader>
                        <CardTitle className="flex items-start gap-3">
                            <Folder className="text-primary mt-1 flex-shrink-0"/> 
                            <span className="flex-1 line-clamp-2">{project.name}</span>
                        </CardTitle>
                      </CardHeader>
                      <CardContent className="flex-grow">
                          <CardDescription>
                            {isOwner ? "You are the owner of this project." : "You are a member of this project."}
                          </CardDescription>
                      </CardContent>
                      <CardFooter className="bg-muted/50 p-3 mt-4">
                          <div className="text-sm text-muted-foreground flex items-center gap-2">
                            {isOwner ? <Crown className="h-4 w-4 text-amber-500" /> : <Users className="h-4 w-4" />}
